...
pub enum Status {
    ...
    Running,
    ReadyForVerification,    // execution finished, ready to get verified
    ReadyToHarvest,          // verified and ready to harvest and pay!
    ExecutionFailed,         
    VerificationFailed,      
...
impl DockerProcessStream {
    ...
    pub fn add(&mut self, job_id: String, image: String, cmd: String) -> Result<(), Error>{
        // let cmd_args = vec![
        //     "run", "--name", "abcd",
        //     "--mount source=aloha-vol,target=/root/proofs",
        //     "test-risc0", "sh", "-c",
        //     "/root/risc0-0.17.0/examples/target/release/factors"
        // ];

        let container_name = format!("--name={job_id}");
        let mount = format!("--mount=source={job_id},target=/root/residue");
